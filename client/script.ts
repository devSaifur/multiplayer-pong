...
function drawScore() {
    ...
    if (!gameOver) {
        ...
        ctx.fillText(
            `Winner: ${winner}, Please refresh to start over`,
            canvas.width / 2 - 200,
            canvas.height / 2 - 25
        )
    }
...
function updateBall() {
    ...
    function increaseSpeedOnCornerHit(ballX: number, paddleX: number) {
...
function mouseControl(event: MouseEvent) {
